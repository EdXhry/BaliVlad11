"""
Парсер сайта https://venasobali.com.au - недвижимость на Бали.
"""
import logging
import re
...
class VenasoBaliParser(BaseParser):
    """
    Парсер сайта https://venasobali.com.au
    Сайт о недвижимости на Бали.
    """
    
    SOURCE_URL = "https://venasobali.com.au"
    SOURCE_NAME = "Venaso Bali Real Estate"

    def parse(self) -> list[RawEvent]:
        """
        Парсим мероприятия с сайта venasobali.com.au
        """
        ...
        try:
            ...
            event_elements = soup.select(
                "article, .post, .entry, .event-item, "
                "div[class*='event'], div[class*='post'], "
                "a[href*='/event'], a[href*='/news'], a[href*='/blog']"
            )
    ...
    def _parse_element(self, element) -> Optional[RawEvent]:
        ...
        try:
            ...
            desc_el = element.select_one(
                ".excerpt, .summary, .entry-content, p, [class*='desc']"
            )
